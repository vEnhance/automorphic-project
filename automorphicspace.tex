...
Another way to give a Hodge structure is related to the representation of the {\it Deligne torus} $\mathbb S$, which is defined to be the restriction of scalars $\rm Res_{\mathbb C/ \mathbb R} \mathbb G_m$, as a linear algebraic group over $\mathbb R$.
...
\item A {\it holomorphic family of Hodge structures} on a connected complex manifold $S$ is a continuous family of Hodge structures $(h_s)_{s \in S}$ on a real vector space $V$, such that the map $\phi$ from $S$ to flag varieties defined by
$$
